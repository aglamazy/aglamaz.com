 ...
 *
 * Per spec-periodic-tests.md, this catches the class of bug where a var whose consuming
 * code gracefully no-ops when unset (so nothing crashes) can still be a real gap nobody
 * would otherwise notice - AGENTS_OBSERVE_* was the original motivating example
 * (confirmed unset in prod 2026-08-16) but its consuming code was reverted 2026-08-19
 * (no working Vercel install path for that private git dependency - see the revert
 * commit), so it's not tracked here until that adoption actually lands. This is a
 * CURATED list, not a blind scan of every `process.env.X` reference in the repo -
 * several real env vars are legitimately optional by design (OPENAI_API_KEY gates
 * BlogAutogenService.isEnabled(), WABA_ vars known-absent per famcircle#142, DROPBOX_
 * and GENI_ vars are separate integrations not core to the app, JWT_ISSUER/JWT_KID both
 * have explicit `|| undefined`-shaped fallback handling in src/auth/tokens.ts:43 and
 * src/auth/jwt.ts:59 - confirmed both genuinely unset in prod 2026-08-16 and auth still
 * works, so they're correctly optional, unlike JWT_PRIVATE_KEY/JWT_PUBLIC_KEY which have
 * no fallback) and would just be noise here. Only vars whose ABSENCE is a real bug
 * belong in EXPECTED_PROD_ENV_VARS.
 */
