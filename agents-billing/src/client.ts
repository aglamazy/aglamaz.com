import type {
  ...
  PaymentStatus,
  PaymentStatusClient,
  PaymentStatusClientConfig,
  PaymentStatusQueryArgs,
} from "./types.js";
...
function buildStatusUrl(args: Pick<PaymentStatusQueryArgs, "baseUrl" | "customer">): string {
...
function isPaymentStatusKind(value: unknown): value is PaymentStatus["kind"] {
...
function parsePaymentStatus(value: unknown): PaymentStatus {
  ...
  if (
    typeof value.period_length_days !== "number" ||
...
async function requestPaymentStatus(args: PaymentStatusQueryArgs): Promise<PaymentStatus> {
  if (!args.customer) {
  ...
  const fetchImpl: FetchLike = args.fetch ?? fetch;
  ...
  if (!res.ok) {
    ...
    throw new Error(
      `agents-billing: billing status request failed (${res.status} ${res.statusText})${body ? `: ${body}` : ""}`,
    );
  }
...
export function createPaymentStatusClient(
  config: PaymentStatusClientConfig,
): PaymentStatusClient {
  return {
    getPaymentStatus() {
