...
import { normalizeSlug, suggestSlugFromParts } from '@/utils/slug';
...
export default function BlogSetupModal({ open, onClose, onSuccess }: BlogSetupModalProps) {
  ...
  const suggestion = useMemo(() => {
    ...
    const fromMember = suggestSlugFromParts(member?.displayName, member?.firstName, member?.email?.split('@')[0]);
    if (fromMember) return fromMember;
    const fromUser = suggestSlugFromParts(user?.name, user?.email?.split('@')[0]);
    return fromUser || normalizeSlug(user?.user_id || '');
  }, [member?.blogHandle, member?.displayName, member?.firstName, member?.email, user?.email, user?.name, user?.user_id]);
